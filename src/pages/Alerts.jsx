  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid ' + (dark ? '#ffffff10' : '#e2e8f0'), marginBottom: 16 }}>
        {['overview', 'rules', 'history', 'analytics'].map(t => <button key={t} style={st.tab(tab === t)} onClick={() => setTab(t)}>{t.charAt(0).toUpperCase() + t.slice(1)}</button>)}
      </div>
      {tab === 'overview' && <>
        <div style={st.grid(4)}>
          <StatCard label="Critical" value={counts.Critical} color="#ef4444" icon="🔴" onClick={() => setFilter('Critical')} />
          <StatCard label="High" value={counts.High} color="#f97316" icon="🟠" onClick={() => setFilter('High')} />
          <StatCard label="Medium" value={counts.Medium} color="#eab308" icon="🟡" onClick={() => setFilter('Medium')} />
          <StatCard label="Low" value={counts.Low} color="#3b82f6" icon="🔵" onClick={() => setFilter('Low')} />
        </div>
        <Card>
      ...
      {tab === 'rules' && <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}><div style={{ fontWeight: 600 }}>Alert Rules</div><Btn>+ Add Rule</Btn></div>
        <DataTable cols={[
          { key: 'name', label: 'Rule' }, { key: 'condition', label: 'Condition' }, { key: 'threshold', label: 'Threshold' },
          { key: 'severity', label: 'Severity', render: v => <span style={styles(dark).badge(styles(dark).severityColor[v])}>{v}</span> },
          { key: 'enabled', label: 'Status', render: v => <span style={styles(dark).badge(v ? '#10b981' : '#64748b')}>{v ? 'Enabled' : 'Disabled'}</span> }
        ]} rows={[
          { name: 'High CPU', condition: 'CPU > threshold', threshold: '85%', severity: 'High', enabled: true },
          { name: 'Memory Critical', condition: 'Memory > threshold', threshold: '95%', severity: 'Critical', enabled: true },
          { name: 'Offline Alert', condition: 'No heartbeat', threshold: '5 min', severity: 'Critical', enabled: true },
          { name: 'Disk Warning', condition: 'Disk > threshold', threshold: '80%', severity: 'Medium', enabled: false }
        ]} />
      </Card>}
