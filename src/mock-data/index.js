export const DEVICES = Array.from({ length: 24 }, (_, i) => {
  const org = ['Org Alpha', 'Org Beta', 'Org Gamma'][i % 3];
  const country = ['Germany', 'USA', 'India', 'Japan'][i % 4];
  const city = {
  ...
  const location = ['Factory A', 'Warehouse B', 'Main Office', 'Site D'][i % 4];
  const withinLocation = ['Section 1', 'Section 2', 'Floor 1', 'Floor 2'][i % 4];
  ...
  return {
    ...
    org, country, city, location, withinLocation,
    lastSeen: i % 5 === 0 ? '2h ago' : (i + 1) + 'm ago',
    firmware: 'v2.' + (i % 4) + '.' + (i % 3), ip: '192.168.1.' + (100 + i),
  };
});

export const BRANDS = ['Bosch', 'Siemens', 'Honeywell', 'ABB'];
...
export const FILES = [
  ...
  { id: 5, name: 'firmware_v4.2.zip', category: 'Firmware', size: '12 MB', version: '4.2', uploaded: '3d ago', status: 'Active' },
];
...
export const telemetryStats = { totalProductProduced: 14872, avgProductPerDay: 621, criticalErrorCount: 7, totalAlerts: ALERTS.length };
