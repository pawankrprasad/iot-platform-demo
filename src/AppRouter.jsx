...
const Reports = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.Reports })));
const GenerateReport = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.GenerateReport })));
const ScheduledReports = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.ScheduledReports })));
const FileManagement = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.FileManagement })));
const UploadFile = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.UploadFile })));
const SelectTargets = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.SelectTargets })));
const UserManagement = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.UserManagement })));
const AddUser = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.AddUser })));
const OrgManagement = lazy(() => import('./pages/OtherPages').then((m) => ({ default: m.OrgManagement })));
...
function Shell() {
  ...
  const signOut = () => {
    ...
    // TODO: implement real sign out logic (auth state cleanup, redirect, etc.)
            ...
            {userMenuOpen && (
              <div style={{ position: 'absolute', right: 0, top: 38, width: 220, background: dark ? '#1e2535' : '#fff', border: '1px solid ' + (dark ? '#ffffff1f' : '#e2e8f0'), borderRadius: 8, boxShadow: '0 8px 16px rgba(0,0,0,0.15)', zIndex: 20 }}>
                <div style={{ padding: 12, borderBottom: '1px solid ' + (dark ? '#ffffff0f' : '#f1f5f9') }}>
                  ...
                  <div style={{ fontSize: 12, color: '#94a3b8' }}>admin@iotplatform.com</div>
                </div>
